      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          ...
          <div className="text-center mb-6">
            <div className={`text-4xl font-bold mb-2 ${getPerformanceColor(easyPercentage)}`}>
              {Math.round(easyPercentage)}%
            </div>
            <p className="text-gray-600">Cards marked as "Easy"</p>
            <p className="text-lg text-gray-700 mt-2">
              {getPerformanceMessage(easyPercentage)}
            </p>
          </div>
          ...
          <div className="w-full bg-gray-200 rounded-full h-4 mb-4">
            <div className="flex h-4 rounded-full overflow-hidden">
              <div 
                ...
                style={{ width: `${(stats.easy / summary.totalCards) * 100}%` }}
              ></div>
              <div 
                className="bg-yellow-500"
                style={{ width: `${(stats.difficult / summary.totalCards) * 100}%` }}
              ></div>
              <div 
                className="bg-red-500"
                style={{ width: `${(stats.incorrect / summary.totalCards) * 100}%` }}
              ></div>
            </div>
          </div>
                        ...
                        {card.imageUrl ? (
                          <img
                            src={card.imageUrl.startsWith('/uploads/') 
                              ? `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}${card.imageUrl}`
                              : card.imageUrl
                            }
                            alt={card.birdName}
